...
import { ORDER_PROCESS_STATUS_ENUM, ORDER_STATUS_ENUM, PAYMENT_STATUS_ENUM, WEB_HOOKS_ENUM } from "../../utils/enums.js"
...
export const last_seven_days_order_invoices = async (req: Request, res: Response) => {
    try {
...
export const get_seller_orders = async (req: Request, res: Response) => {
    try {
...
export const get_buyer_orders = async (req: Request, res: Response) => {
    try {
        let { limit, page, title, start_date, end_date } = req.query as unknown as {
            page: number,
            limit: number
            title: string
            start_date: string
...
export const change_order_process_status = async (req: Request, res: Response) => {
    try {
...
export const seller_subscriptions_orders = async (req: Request, res: Response) => {
    try {
        let { limit, page, start_date, end_date, subscription } = req.query as unknown as {
            page: number,
            limit: number
            start_date: string
            end_date: string
            subscription: SubscriptionNameEnum
        }
        page = page ?? 1
        limit = limit ?? 5
        const skip = (page - 1) * limit
        ...
        console.log(results)
        const confirmed_invoices_pipeline: PipelineStage[] = [
            {
                $match: {
                    seller: new mongoose.Types.ObjectId(uid),
                    payment_status: PAYMENT_STATUS_ENUM.PAID
...
export const buyer_invoices = async (req: Request, res: Response) => {
    try {
...
export const get_buyer_analytics = async (req: Request, res: Response) => {
    try {
