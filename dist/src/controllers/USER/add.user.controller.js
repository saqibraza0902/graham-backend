...
export const getSingleAddForDescriptionPage = async (req, res) => {
    try {
        const { id } = req.params;
        const dbAdd = await Add.findById(id).populate("created_by", '-password');
        if (!dbAdd) {
...
const buildSearchAddsQuery = (queries) => {
    if (!queries) {
        return {};
    }
    let searchQuery = {};
...
export const searchAdds = async (req, res) => {
    try {
...
export const disableAdd = async (req, res) => {
    try {
        const { id } = req.params;
        const dbAdd = await Add.findById(id);
        if (!dbAdd) {
...
export const unableAdd = async (req, res) => {
    try {
        const { id } = req.params;
        const dbAdd = await Add.findById(id);
        if (!dbAdd) {
...
export const getDescriptionPageAddWithDetails = async (req, res) => {
    try {
        const { id } = req.params;
